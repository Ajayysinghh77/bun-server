...
### Phase 1: 数据库集成 (v0.4.0)

**目标**：提供完整的数据库集成能力，这是企业级应用的核心需求

#### 1.1 数据库连接管理（DatabaseModule）

- [ ] 实现 DatabaseModule（从 v0.3.0 Phase 3.1 迁移）
  - [ ] 连接池管理（基于 `bun:sqlite`、`postgres`、`mysql2`）（从 v0.3.0 Phase
        3.1 迁移）
  - [ ] 多数据库支持（PostgreSQL, MySQL, SQLite）（从 v0.3.0 Phase 3.1 迁移）
  - [ ] 连接健康检查（从 v0.3.0 Phase 3.1 迁移）
  - [ ] 连接重试机制（从 v0.3.0 Phase 3.1 迁移）
  - [ ] 连接池配置（最大连接数、超时时间等）
...
#### 1.2 ORM 集成（Drizzle ORM）

- [ ] 选择并集成 ORM（推荐 Drizzle）（从 v0.3.0 Phase 3.2 迁移）
  - [ ] Drizzle ORM 集成（从 v0.3.0 Phase 3.2 迁移）
  - [ ] 装饰器支持（@Entity, @Column, @PrimaryKey）（从 v0.3.0 Phase 3.2 迁移）
  - [ ] Repository 模式支持（从 v0.3.0 Phase 3.2 迁移）
  - [ ] 查询构建器集成（从 v0.3.0 Phase 3.2 迁移）
  - [ ] 关系映射支持（@OneToMany, @ManyToOne, @ManyToMany）
- [ ] 提供示例和文档（从 v0.3.0 Phase 3.2 迁移）
...
- [ ] 实现事务装饰器 `@Transactional()`
  - [ ] 基本事务支持（从 v0.3.0 Phase 3.4 迁移）
  - [ ] 嵌套事务支持（从 v0.3.0 Phase 3.4 迁移）
  - [ ] 事务传播行为配置（REQUIRED, REQUIRES_NEW, NESTED）（从 v0.3.0 Phase 3.4
        迁移）
  - [ ] 事务隔离级别配置
  - [ ] 回滚策略配置
- [ ] 集成到 DatabaseModule
...
- [ ] 完善错误码系统
  - [ ] 统一错误码规范
  - [ ] 错误码文档
  - [ ] 添加错误码系统（从 v0.3.0 Phase 1.3 迁移）
- [ ] 改进错误消息国际化
  - [ ] 支持多语言错误消息
  - [ ] 错误消息模板系统
  - [ ] 改进错误消息国际化（从 v0.3.0 Phase 1.3 迁移）
- [ ] 完善异常过滤器文档
- [ ] 添加错误处理最佳实践
...
- [ ] 数据库集成完整（Database, ORM, Transaction）
