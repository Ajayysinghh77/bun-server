...
## 新增特性提案：HeaderMap 注解能力

### 背景

- 现状：`@Header()` 仅支持读取单个
  header；业务常需一次性获取全部请求头或按需过滤。
- 问题：需手动从 `Context` 逐个读取，缺少类型与多值（同名多值）处理的统一入口。

### 目标

- 提供 `@HeaderMap()`（或等价装饰器），在控制器方法参数处注入完整 header 对象。
- 兼容单值/多值场景，可选对 header key 做规范化（小写）与过滤。

### 设计要点

- 返回形态：默认 `Record<string, string | string[]>`；对同名多值支持数组聚合。
- 兼容装饰器参数：允许传入选项（如
  `{ normalize?: boolean; pick?: string[]; transform?: (input) => T; validate?: (dto) => void }`）。
- ParamBinder 集成：新增 `ParamType.HEADER_MAP` 分支，与 `@Header()`
  并行不互斥；可选择是否对 header 名做小写规范化。
- 类型定义：`HeaderMapOptions`，支持泛型 `@HeaderMap<T>()`。

### 交付里程碑（v1.3.x 内）

- [ ] 装饰器实现：`@HeaderMap()` + `ParamType.HEADER_MAP`
- [ ] ParamBinder 支持：聚合 headers → 对象（支持多值、可选规范化/筛选）
- [ ] 选项/类型：`HeaderMapOptions`（normalize/pick/transform/validate 可选）
- [ ] 导出：`src/controller` 及根入口统一导出
- [ ] 文档与示例：在示例或 README 中演示 `@HeaderMap()` 用法
- [ ] 测试：单元测试（聚合/多值/normalize/pick/transform/validate）；控制器集成测试

### 验收标准

- 控制器方法可通过 `@HeaderMap()` 一次注入完整 headers，对同名多值可按设计聚合。
- 可选 normalize/pick/transform/validate
  逻辑在绑定阶段生效（验证失败抛出对应异常）。
- 不影响现有 `@Header()` 等参数绑定行为；全量测试通过。
